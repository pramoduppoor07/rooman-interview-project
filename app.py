...
st.caption("Upload a PDF or select a sample — see raw text, extracted fields, and validation flags.")
...
with st.sidebar:
    st.header("Input")
    mode = st.radio("Source", ["Upload PDF", "Use sample file"])
    ...
    if mode == "Upload PDF":
        ...
        if uploaded:
            tmp = Path("results") / "_upload_tmp.pdf"
        ...
        if samples:
            ...
            pdf_path = SAMPLES_DIR / choice
        else:
            st.warning("No sample files found. Run `python -m src.generator.generate_samples` first.")

    run = st.button("Extract", type="primary", disabled=(pdf_path is None))
